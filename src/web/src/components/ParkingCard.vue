  ...
  <v-card class="mb-3" elevation="1">
    <v-card-item class="clickable-header" @click="navigateToSuburb">
      ...
      <template #append>
        ...
        >
          <v-icon :color="isFavourite ? 'yellow-darken-2' : 'grey-lighten-1'">
    ...
    <v-card-text class="pt-3 pb-2">
      <div class="d-flex flex-wrap gap-1">
        <v-chip size="small" :color="getTypeColor(submission.type)" label>
          <v-icon start size="14">{{ getTypeIcon(submission.type) }}</v-icon>
          {{ formatParkingType(submission.type) }}
        </v-chip>

        <v-chip v-if="submission.lighting" size="small" color="success" label>
        ...
        >
          ...
          CCTV:
          {{ submission.cctv === true || submission.cctv === 1 ? 'Yes' : 'No' }}
        </v-chip>
      </div>
    </v-card-text>
    ...
    <v-card-actions class="d-flex justify-space-between align-center px-4">
      <div
        v-if="submission.facilities && submission.facilities.length > 0"
...
<script setup lang="ts">
  ...
  const isFavourite = computed(() =>
    favourites.value.has(props.submission.parking_id)
  );
